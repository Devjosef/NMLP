...
def run_once(engine_args):
    ...
    if args.csv: Path(args.csv).write_text(result.get("csv", ""))
    if not args.quiet: print(result.get("summary", ""))
    ...
    def monitor_target_worker(target):
        ...
        while True:
            ...
            try:
                ...
                if isinstance(raw_loss, str): raw_loss = float(raw_loss.replace("%", "").strip())
                
                metrics = {
                    "loss_pct": raw_loss or 0.0,
                    "latency_ms": result.get("latency_ms", 0.0) or 0.0,
                    "jitter_ms": result.get("jitter_ms", 0.0) or 0.0
                }
                summary = result.get("summary", "") or ""
                is_alert = metrics["loss_pct"] > 5.0 or "BAD" in summary or "(S:0/" in summary or "100.0%" in summary
                if "ICMP: 0%" in summary or "ICMP base: 0.0%" in summary: is_alert = False
            ...
            time.sleep(max(0.1, interval - (time.time() - worker_start)))
...
def generate_isp_report(incident_id: int):
    ...
    report_data = {
        "id": row[0], "timestamp": row[1], "target": row[2], "summary": row[3],
